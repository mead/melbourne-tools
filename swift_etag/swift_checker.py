...
def swift_client(config_file):
    if config_file:
        ...
        auth_vars = (auth_username, auth_password, auth_tenant_id,
                     auth_tenant_name, auth_region_name, auth_url)
        ...
        auth_vars = (os.environ['OS_AUTH_URL'], os.environ['OS_REGION_NAME'],
                     os.environ['OS_TENANT_NAME'], os.environ['OS_USERNAME'],
...
def split(filename, segment_size, segment_container):
    ...
    while segment_count < number_of_segments:
        ...
        while read_bytes < segment_size:
            ...
            # Adjust the number of bytes to read if we're near the end of the
            # segment
        ...
        if verbose_mode == 2:
            if md5_seg == segment_container[segment_count].object_hash:
                display_output(
                    "Seg: ", segment_count,
                    segment_container[segment_count].object_hash, md5_seg)
...
def arg_handling():
    parser = argparse.ArgumentParser(
        ...
        Single File Comparisons:
            ...
            1. Example: Compare local file with object sharing \
the same name in swift container

               ./swift_checker.py filename container

            2: Example: Compare local file with object having a different\
name or path
        ...
        Directory Comparison:
            ...
            3: Example: Recursively compare all files contained within a\
...
my_objects_start_with/this/prefix/or/path
    ...
    parser.add_argument(
        'container', help='Name of the Swift container to check')
    parser.add_argument('object_or_path', help='''Optional: For file \
                        comparison this is the specific \
...
def main():
    ...
    # complete
                j = [sc, os.path.abspath(
                    os.path.join(root, name)), args.container, cur_object]
                jobs.append(j)
